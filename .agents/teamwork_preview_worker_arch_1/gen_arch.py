...
content = """# Charon System Architecture & Technical Specification Report
...
1. **Signal Ingestion**: Ingests high-speed market events from PumpPortal WebSocket (`wss://pumpportal.fun`), GMGN token signal APIs, Fee Claim monitors, PumpFun pre-grad monitors, and central Signal Server HTTP polling.
...
graph TD
    ...
    subgraph Subsystem_1 ["1. Signals & Ingestion Layer"]
        PP["PumpPortal WebSocket Client<br/>(wss://pumpportal.fun)<br/>New Token & Migration Listener"] :::signalFill
    ...
    subgraph Subsystem_3 ["3. Dynamic Enrichment Layer"]
    ...
    subgraph Subsystem_7 ["7. SQLite Database Layer (charon.sqlite)"]
    ...
    subgraph Subsystem_2 ["2. Core Pipeline Orchestrator & Scoring Engine"]
        ...
        MOM_FILTER["Momentum Filter Subprocess<br/>(predict_momentum.py Scikit-Learn Model)"] :::pipeFill
    end

    subgraph Subsystem_4 ["4. Regime & Macro Intelligence Engines"]
    ...
    subgraph Subsystem_5 ["5. LLM Integration & Decision Consensus Engine"]
    ...
    subgraph Subsystem_6 ["6. Execution Router & Jupiter Executor"]
    ...
    subgraph Subsystem_8 ["8. Auto-Learn & Self-Tuning Engine"]
    ...
    subgraph Subsystem_9 ["9. Telegram UI & Operator Controls"]
    ...
    MACRO -->|Fetch Live SOL/USDT Price| BINANCE["Binance Spot API"]
...
### 4.2 Multi-Source Dynamic Enrichment Engine

The **Dynamic Enrichment Engine** fetches multi-dimensional market, holder, social, and security metrics in parallel before scoring.
...
### 4.3 Core Pipeline Orchestrator & Scoring Engine
...
6. **ML Momentum Prediction (`src/pipeline/momentumFilter.js` & `predict_momentum.py`)**:
...
### 4.4 Regime & Macro Intelligence Engines
...
### 4.5 LLM Integration & Decision Consensus Engine
...
### 4.6 Execution Router & Jupiter Executor
...
### 4.7 SQLite Database Schema & State Locks
...
### 4.8 Auto-Learn & Self-Tuning Engine
...
+---------------------+      +---------------------+      +---------------------+
                                                                     ...
                                                                     v
+---------------------+      +---------------------+      +---------------------+
| Dynamic Settings &  | <--- |  AutoApply Mutator  | <--- |   Recency & Count   |
...
- **Gating Rules**:
  - **Minimum Closed Positions**: Requires $\ge 30$ closed positions in DB.
  ...
  - **Confidence Floor**: Requires lesson confidence score $\ge 0.7$.
...
### 4.9 Telegram UI & Operator Controls
...
target = '/root/Kaiser.charon/charon_architecture.md'
with open(target, 'w', encoding='utf-8') as f:
    f.write(content)

print(f"SUCCESS: Wrote {len(content)} bytes to {target}")
