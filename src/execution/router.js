...
export function assertLiveRiskBudget(nextSizeSol) {
  ...
  `).get();
  if (Number(active.count) >= LIVE_MAX_OPEN_POSITIONS) {
    throw new Error(`Hard live position cap reached (${active.count}/${LIVE_MAX_OPEN_POSITIONS}).`);
  }
  if (Number(active.exposure) + Number(nextSizeSol) > LIVE_MAX_TOTAL_EXPOSURE_SOL) {
    throw new Error(`Hard live exposure cap exceeded (${LIVE_MAX_TOTAL_EXPOSURE_SOL} SOL).`);
  }
  const since = Date.now() - 24 * 60 * 60 * 1000;
  const entries = db.prepare(`
    SELECT count(*) AS count FROM execution_operations
    WHERE side = 'buy' AND status IN ('completed', 'outcome_unknown') AND created_at_ms >= ?
  `).get(since).count;
  if (Number(entries) >= LIVE_MAX_DAILY_ENTRIES) {
    throw new Error(`Hard daily live entry cap reached (${entries}/${LIVE_MAX_DAILY_ENTRIES}).`);
  }
  const pnl = Number(db.prepare(`
    SELECT coalesce(sum(pnl_sol), 0) AS pnl FROM dry_run_positions
    WHERE execution_mode = 'live' AND status = 'closed' AND closed_at_ms >= ?
  `).get(since).pnl);
  if (pnl <= -LIVE_MAX_DAILY_LOSS_SOL) {
    throw new Error(`Hard daily loss limit reached (${pnl.toFixed(4)} SOL).`);
  }
...
export function assertSafeLiveDecision(decision, strat) {
  ...
  if (decision?.verdict !== 'BUY' || !Number.isFinite(confidence) || confidence < 0 || confidence > 100) {
    throw new Error('Invalid live BUY decision or confidence.');
  }
...
export async function executeLiveBuy(selectedRow, decision, batchId, rows = [], triggerCandidateId = null) {
  // Revalidate immediately before every automatic live entry. This enforces
  // snapshot expiry and catches code/model/runtime drift without a restart.
    ...
    try {
      ...
      if (!swap.outputAmount) {
        try {
          ...
          if (after != null && tokenBalanceBefore != null && BigInt(after) >= BigInt(tokenBalanceBefore)) {
            swap.outputAmount = (BigInt(after) - BigInt(tokenBalanceBefore)).toString();
          }
        } catch (balErr) {
      ...
      if (err.swapOutcomeUnknown) {
        console.error(`[executeLiveBuy] outcome unknown after execute submission; refusing automatic retry for ${candidate.token.mint.slice(0, 8)}...`);
        break;
      }
      const fatalErrors = ['insufficient balance', 'insufficient funds', 'slippage'];
      if (err.message && fatalErrors.some(msg => err.message.toLowerCase().includes(msg))) {
  ...
  if (!swap && lastError?.swapOutcomeUnknown && tokenBalanceBefore != null) {
    ...
    if (after != null && BigInt(after) > BigInt(tokenBalanceBefore)) {
      swap = {
        signature: lastError.swapSignature || null,
        inputAmount: String(amountLamports),
        outputAmount: (BigInt(after) - BigInt(tokenBalanceBefore)).toString(),
        recoveredFromBalance: true,
      };
      lastError = null;
      console.warn(`[executeLiveBuy] recovered successful buy from token balance delta for ${candidate.token.mint.slice(0, 8)}...`);
    }
  }
  if (!swap) {
    ...
    logDecisionEvent({
      ...
      execution: { operationId: claim.operationId, outcomeUnknown: unknown, error: lastError?.message || 'unknown' },
    });
    await sendTelegram([
      ...
      unknown ? `Operation #${claim.operationId} requires reconciliation; automatic retry is blocked.` : `Operation #${claim.operationId} recorded as failed.`,
    ].join('\n'));
...
export async function executeLiveSell(position, reason) {
  ...
  if (tokenBalanceBefore != null) {
    if (!String(reason).includes('PARTIAL')) {
      amount = tokenBalanceBefore;
    } else if (BigInt(amount) > BigInt(tokenBalanceBefore)) {
      throw new Error('Partial sell amount exceeds confirmed wallet token balance.');
    }
  }
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      lastError = err;
      console.error(`[executeLiveSell] attempt ${attempt}/3 failed for ${position.mint.slice(0, 8)}... ${err.message}`);
      if (err.swapOutcomeUnknown) {
        console.error(`[executeLiveSell] outcome unknown after execute submission; refusing automatic retry for ${position.mint.slice(0, 8)}...`);
        break;
      }
      if (attempt < 3) {
        await new Promise(r => setTimeout(r, 1500 * attempt));
  ...
  if (lastError?.swapOutcomeUnknown && tokenBalanceBefore != null) {
    ...
    if (after != null && BigInt(tokenBalanceBefore) - BigInt(after) >= expectedDecrease) {
      updateExecutionOperation(claim.operationId, 'completed', {
        ...
        signature: lastError.swapSignature,
        outputAmount: null,
        error: 'recovered_from_token_balance_delta',
      });
      console.warn(`[executeLiveSell] recovered successful sell from token balance delta for ${position.mint.slice(0, 8)}...`);
...
export async function executeConfirmedIntent(chatId, intentId) {
  ...
  try {
    ...
    const amountLamports = Math.floor(scaledSizeSol * 1_000_000_000);
    if (!Number.isSafeInteger(amountLamports) || amountLamports <= 0 || scaledSizeSol > LIVE_MAX_POSITION_SOL) {
      throw new Error(`Unsafe confirmed position size: ${scaledSizeSol} SOL (cap ${LIVE_MAX_POSITION_SOL})`);
    }
    assertLiveRiskBudget(scaledSizeSol);
    ...
    try {
      ...
      updateExecutionOperation(operationId, 'failed', { error: error.message });
      throw error;
    }
    if (balance < amountLamports + LIVE_MIN_SOL_RESERVE_LAMPORTS) {
      updateExecutionOperation(claim.operationId, 'failed', { error: 'insufficient_balance' });
    ...
    if (!swap.outputAmount) {
      try {
        ...
        console.warn(`[executeConfirmedIntent] failed to fetch balance after swap, ignoring: ${balErr.message}`);
      }
    }
    ...
    if (operationId && (err.swapOutcomeUnknown || completedSwap)) {
      updateExecutionOperation(operationId, 'outcome_unknown', {
        signature: completedSwap?.signature,
        outputAmount: completedSwap?.outputAmount,
        error: completedSwap ? `swap_succeeded_position_persist_failed: ${err.message}` : err.message,
      });
...
export async function rejectIntent(chatId, intentId) {
  const rejected = db.prepare(`
    UPDATE trade_intents SET status = 'rejected', updated_at_ms = ?
