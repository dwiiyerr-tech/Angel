...
const report = {
  simulator: 'zero_capital_quote_v1',
  ...
  medianTimeToMfeMinutes: (() => {
    const value = median(rows.map(row => Number(row.time_to_mfe_ms) / 60000));
    return value;
  })(),
...
const normalized = Object.fromEntries(Object.entries(report).map(([key, value]) => [
  key,
  typeof value === 'number' && Number.isFinite(value) ? rounded(value) : value,
]));
